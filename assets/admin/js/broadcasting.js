...
const BASE_URL       = document.querySelector("meta[name=APP-DOMAIN]").getAttribute('content') + '/api/v1';
...
makeAuthEndPointForPusher = (socketId, channelName) => {
    return AUTH_END_POINT.replace(':socketId', socketId).replace(":channelName", channelName)
}

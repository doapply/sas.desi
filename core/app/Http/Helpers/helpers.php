...
{
    $characters = '0ABCDEFGHJKMNOPQRSTUVWXYZ123456789';
    $charactersLength = strlen($characters);
    $randomString = '';
    for ($i = 0; $i < $length; $i++) {
        $randomString .= $characters[rand(0, $charactersLength - 1)];
...
function notify($user, $templateName, $shortCodes = null, $sendVia = null, $createLog = true)
{
    ...
    $notify->userColumn = $createLog ?? $user->getForeignKey();
    $notify->send();
}
...
function apiResponse($success, $statsCode, $message = null, $errors = [], $data = [])
{
    $response['success'] = $success;
    $response['code']    = $statsCode;

    $message != null ? $response['message'] = $message : "";

    gettype($errors) == 'array' &&  count($errors)  > 0 ? $response['errors'] = $errors : '';
    gettype($data) == 'array' &&  count($data)  > 0 ? $response['data'] = $data : '';
...
{
    $batchId         = gs()->batch_id_prefix . '-' . ((Batch::max('id') ?? 0) + 1);
    $batch           = new Batch();
